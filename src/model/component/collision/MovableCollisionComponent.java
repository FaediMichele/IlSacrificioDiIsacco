...
public class MovableCollisionComponent extends CollisionComponent {
    ...
    protected void handleCollision(final CollisionEvent event) {
        ...
        if (event.getSourceEntity().getComponent(BodyComponent.class).get().isPhysics()
                && this.getEntity().getComponent(BodyComponent.class).get().isPhysics()) {
    ...
    protected void handleMovement(final CollisionEvent event) {
        ...
        final Pair<Double, Double> p1 = new Pair<>(b1.getPosition().getX() + b1.getWidth() / 2,
                b1.getPosition().getY() + b1.getHeight() / 2);
        final Pair<Double, Double> p2 = new Pair<>(b2.getPosition().getX() + b2.getWidth() / 2,
                b2.getPosition().getY() + b2.getHeight() / 2);
    ...
    private void reactToCollision(final Entity other) {
        ...
        if (touchDown(mine, otherBody) && touchRight(mine, otherBody)) {
            if (mine.getPosition().getX() + mine.getWidth() - otherBody.getPosition().getX()
                    - initVelX < mine.getPosition().getY() + mine.getHeight() - otherBody.getPosition().getY()
                            - initVelY) {
            ...
            if (mine.getPosition().getY() + mine.getHeight() - otherBody.getPosition().getY()
                    - initVelY > otherBody.getPosition().getX() + otherBody.getWidth() - mine.getPosition().getX()
                            - initVelX) {
            ...
            if (mine.getPosition().getX() + mine.getWidth() - otherBody.getPosition().getX()
                    + initVelX < otherBody.getPosition().getY() + otherBody.getHeight() - mine.getPosition().getY()
                            + initVelY) {
                newX = rectifyRight(mine, otherBody);
            } else {
            ...
            if (otherBody.getPosition().getY() + otherBody.getHeight() - mine.getPosition().getY()
                    + initVelY > otherBody.getPosition().getX() + otherBody.getWidth() - mine.getPosition().getX()
                            + initVelX) {
                newX = rectifyLeft(otherBody);
            } else {
